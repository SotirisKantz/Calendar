...
function myfunction(day, month, year, currentMonth, currentYear) {
  ...
  var dayOfWeek = d.getDay(); //const DaysBeforeFirst = ((day - dayOfWeek) % 7) - 1; // number of days before the first of the month
  //const numberBefore = 7 - dayOfWeek - 1;

  findMOnthBefore(dayOfWeek, month, year);
  findMOnth(month, year);
  var Array = ['January', 'February', 'March', 'April', 'May', 'June', 'Jule', 'August', 'September', 'October', 'November', 'December'];
  var nameOfMonth = Array[month]; //document.getElementsByClassName('month_year').innerText = month + ' ' + year;
...
function addNodesToDates(number) {
  for (var i = 1; i <= number; i++) {
    document.getElementById('days').innerHTML += "<div class='day'>".concat(i, "</div>");
...
function addDaysBeforeFirst(number, dayOfWeek) {
  for (var i = number - dayOfWeek + 1; i <= number; i++) {
    document.getElementById('days').innerHTML += "<div class='day dayBefore'>".concat(i, "</div>");
  }
}
